                ...
                <div class="overflow-y-auto">
                    @if($histories->count() > 0)
                        @foreach ($histories as $history)
                            <x-timeline-item title="{{ $history->action }}, {{ $history->name }}" subtitle="{{ $history->created_at->diffForHumans() }}" description="by {{ $history->user->fullname }}"  />
                        @endforeach
                    @else
